              ...
              <div class="card">
                @auth
                  <form action="{{ route('booking.store') }}" method="post">
                    @csrf 
                    <input type="hidden" name="project_package_id" value="{{ $project_package->id }}">
                    <input type="text" name="name" placeholder="Nama Anda" required />
                    <input type="email" name="email" placeholder="Email Anda" required />
                    <input type="number" name="number_phone" placeholder="No Telepon" required />
                    <input
            ...
            <article class="popular__card">
                <a href="{{ route('project_package.show', $project_package->slug) }}" class="text-decoration-none">
                    ...
                    <div class="popular__data">
                        <h2 class="popular__price"><span>Rp.</span>{{ number_format($project_package->price) }}</h2>
